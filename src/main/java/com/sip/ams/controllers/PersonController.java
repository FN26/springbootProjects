...
public class PersonController {
  ...
  /// update person method
    ...
    @RequestMapping(value = { "/updatePerson/{fname}/{lname}/{id}" }, method = RequestMethod.GET)
    public String openupdatePerson(@PathVariable String fname, @PathVariable String lname, @PathVariable int id, Model model) {
 
    	Person personObj = new Person(id,fname, lname);
		model.addAttribute("personObj", personObj);
		persons.remove(personObj);
               
        return "Person/updatePerson";
        
    }
    
    @RequestMapping(value = { "/updatePerson/{fname}/{lname}/{id}" }, method = RequestMethod.POST)
    public String updatePerson(@PathVariable String fname, @PathVariable String lname, @PathVariable int id, Model model) {
		
		if (fname != null && fname.length() > 0 //
				&& lname != null && lname.length() > 0) {
			Person newPerson = new Person(id,fname, lname);
			persons.add(newPerson);
			return "redirect:/personList";
		}
		model.addAttribute("errorMessage", errorMessage);
